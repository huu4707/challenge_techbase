...
describe('Get medicine', () => {
  ...
  test('Missing token must be fail', async () => {
    const result = await callAPI({ method: 'get', url: `/medicine/get?medicineId=${medicineId}` });
  ...
  test('Missing medicineId must be fail', async () => {
    ...
    const result = await callAPI({ method: 'get', url: `/medicine/get`, headers });
    expect(result.response).toMatchObject(expectedFailedResult);
  });
  ...
  test('Successful case', async () => {
    const headers: IHeaders = { authorization: `Bearer ${token}` };
    const result = await callAPI({ method: 'get', url: `/medicine/get?medicineId=${medicineId}`, headers });
